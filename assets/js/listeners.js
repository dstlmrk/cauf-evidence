...
function memberForm() {
    return {
        ...
        init() {
            ...
            this.$watch("birthDate", (value) => {
                this.handleBirthDate(value);
            });

            if (this.birthDate === "") {
                this.$refs.form.querySelector("#id_birth_date").value = this.birthDate;
            }
        },
        ...
        handleBirthDate(value) {
            this.$refs.form
                .querySelectorAll(".legal-guardian-field input")
                .forEach((input) => (input.disabled = isOlderThan15(value)));
        },
    };
}
