...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        members_with_original_id_as_null = Member.objects.filter(original_id__isnull=True)
        self.stdout.write(
            f"There is {members_with_original_id_as_null.count()} members with original_id as NULL"
        )
        members_with_original_id_as_empty_str = Member.objects.filter(original_id="")
        self.stdout.write(
            f"There is {members_with_original_id_as_empty_str.count()} members with "
            "original_id as empty string"
        )

        members_with_original_id = Member.objects.filter(original_id__isnull=False).exclude(
            original_id=""
        )
